/*!
	@file    ST7735_LTSM.cpp
	@author  Gavin Lyons
	@brief   Library source file for st7735 display for arduino, Arduino library ST7735_LTSM
...
/*!
	@brief Constructor for class ST7735_LTSM
*/
ST7735_LTSM:: ST7735_LTSM(){}

/*!
	@brief : Init Hardware SPI
*/
void ST7735_LTSM::TFTSPIInitialize(void)
{
	SPI.begin();
}
 ...
 */
void ST7735_LTSM::setAddrWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
...
*/
void ST7735_LTSM::setupGPIO_SPI(uint32_t speed_hz, int8_t rst, int8_t dc, int8_t cs)
{
...
*/
void ST7735_LTSM::setupGPIO_SPI(uint16_t CommDelay, int8_t rst, int8_t dc, int8_t cs, int8_t sclk, int8_t din)
{
...
*/
void ST7735_LTSM::TFTST7735S80160Initialize()
{
	TFTResetPIN();
	DISPLAY16_DC_SetLow;
	DISPLAY16_DC_SetDigitalOutput;
...
*/
void ST7735_LTSM::setScrollDefinition(uint8_t top_fix_heightTFT, uint8_t bottom_fix_heightTFT, bool _scroll_direction) {
...
*/
void ST7735_LTSM::TFTVerticalScroll(uint8_t vsp) {
...
*/
void ST7735_LTSM::TFTchangeInvertMode(bool invertModeOn) {
	if(invertModeOn) {
 ...
 */
void ST7735_LTSM::TFTchangeMode(TFT_modes_e mode) {
	switch (mode) {
		case TFT_Normal_mode:
			...
			if (_currentMode == TFT_Idle_mode) {//was in idle?
				writeCommand(ST7735_Idle_modeOFF);
			}
		...
		case TFT_Partial_mode:
			...
			break;
		case TFT_Idle_mode:
			writeCommand(ST7735_Idle_modeON);
...
*/
void ST7735_LTSM::setRotation(display_rotate_e mode) {
	uint8_t madctl = 0;
	switch (mode) {
		case Degrees_0 :
...
*/
void ST7735_LTSM :: TFTInitScreenSize(uint8_t colOffset, uint8_t rowOffset, uint16_t width_TFT, uint16_t height_TFT)
{
	_colstart = colOffset;
	_rowstart = rowOffset;
	_XStart = colOffset;
...
*/
void ST7735_LTSM ::TFTInitPCBType(TFT_PCBtype_e pcbType)
{
...
*/
uint16_t ST7735_LTSM::TFTSwSpiGpioDelayGet(void){return _SWSPIGPIODelay;}

/*!
	@brief Freq delay used in SW SPI setter, uS delay used in SW SPI method
	@param CommDelay The GPIO communications delay in uS
*/
void  ST7735_LTSM::TFTSwSpiGpioDelaySet(uint16_t CommDelay){_SWSPIGPIODelay = CommDelay;}
//**************** EOF *****************
